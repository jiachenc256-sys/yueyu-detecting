...
export type EmotionId = "grief" | "urgency" | "anger" | "tender" | "heroic" | "sigh";
...
export const EMOTION_AXES: EmotionLabel[] = [
...
export interface ProsodyFeatures {
  ...
  rmsStd: number;
  centroidMean: number;
  pitchProxyMean: number;
  pitchProxyStd: number;
  attackiness: number;
  fluxMean: number;
  voicedRatio: number;
}
...
export function extractProsodyFeatures(samples: Float32Array, sampleRate: number): ProsodyFeatures {
  ...
  const rmsArr: number[] = [];
  const centArr: number[] = [];
  const pitchArr: number[] = [];
  const attackArr: number[] = [];
  ...
  for (let i = 0; i + frame < samples.length; i += hop) {
    ...
    if (rms > 0.02 && zcr > 0.02 && zcr < 0.35) voicedFrames++;
    frames++;
    prevRms = rms;
  }
...
const PIECE_EMOTION_PRIOR: Record<string, Partial<Record<EmotionId, number>>> = {
...
export function featuresToEmotions(
  f: ProsodyFeatures,
  pieceId?: string | null,
): ProsodyResult {
  ...
  if (prior) {
    for (const [k, v] of Object.entries(prior)) {
      const id = k as EmotionId;
      if (id in scores && typeof v === "number") scores[id] = clamp01(scores[id] + v);
    }
  }

  // Softmax-ish sharpen so radar isn't flat
  const maxS = Math.max(...Object.values(scores), 1e-6);
  for (const id of Object.keys(scores) as EmotionId[]) {
  ...
  const label = (id: EmotionId) => EMOTION_AXES.find((a) => a.id === id)?.labelZh ?? id;
