...
export async function loadFingerprintIndex() {
    if (cached)
        return cached;
    if (loadPromise)
        return loadPromise;
    loadPromise = (async () => {
        try {
            ...
            if (!res.ok)
                throw new Error(`audio-fp HTTP ${res.status}`);
...
export function fingerprintFromSamples(samples, bins = 24) {
    const vec = [];
...
export async function fingerprintFromAudioUrl(url, maxSec = 8) {
    try {
...
export function matchFingerprint(query, index, pieceId, topK = 3) {
    const hits = [];
    for (const item of index.items) {
        if (pieceId && item.pieceId !== pieceId)
            continue;
...
//# sourceMappingURL=speak-fingerprint.js.map