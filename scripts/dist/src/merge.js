...
export function computeCoverage(cues) {
    let enCurated = 0;
    let enMt = 0;
    let enAny = 0;
...
export function indexTranslations(file) {
    const map = new Map();
    for (const entry of file.entries) {
        ...
        const preferred = preferEnLayer({ text: existing.text, status: existing.status, ...(existing.source ? { source: existing.source } : {}) }, { text: entry.text, status: entry.status, ...(entry.source ? { source: entry.source } : {}) });
        map.set(entry.id, {
            id: entry.id,
            text: preferred.text,
 ...
 */
export function mergeLayers(base, curated, mt) {
    const cues = ("schemaVersion" in base
        ? base.cues
        : base.cues.map(legacyCueToLayered)).map((cue) => ({
        ...cue,
        layers: { ...cue.layers, zh: { ...cue.layers.zh } },
    }));
    const curatedIndex = curated ? indexTranslations(curated) : new Map();
    const mtIndex = mt ? indexTranslations(mt) : new Map();
    for (const cue of cues) {
    ...
    const correctedCount = cues.filter((c) => c.layers.zh.status === "corrected" || c.layers.zh.status === "reviewed").length;
    const piece = {
        id: base.id,
        title: base.title,
