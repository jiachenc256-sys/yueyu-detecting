...
let cachedPinyin = null;
let pinyinPromise = null;
let cachedPieceRadar = null;
let pieceRadarPromise = null;
const THEME_ORDER = [
    "farewell",
    "longing",
    "exam",
...
function looksDegenerate(hyp) {
    const t = normChars(hyp);
    if (t.length < 2)
        return true;
    const uniq = new Set(t.split("")).size;
    if (t.length >= 4 && uniq / t.length < 0.35)
        return true;
    return false;
}
export async function loadLyricIndex() {
    if (cachedIndex)
        return cachedIndex;
    if (loadPromise)
        return loadPromise;
    loadPromise = (async () => {
        try {
            ...
            if (!res.ok)
                throw new Error(`lyric-index HTTP ${res.status}`);
...
export async function loadSceneCards() {
    ...
    scenesPromise = (async () => {
        try {
            ...
            if (!res.ok)
                throw new Error(`scene-cards HTTP ${res.status}`);
...
export async function loadPinyinMap() {
    if (cachedPinyin)
        return cachedPinyin;
    if (pinyinPromise)
    ...
    pinyinPromise = (async () => {
        try {
            ...
            if (!res.ok)
                throw new Error(`pinyin-map HTTP ${res.status}`);
            cachedPinyin = (await res.json());
            return cachedPinyin;
        }
        catch {
            cachedPinyin = null;
...
export async function loadPieceRadar() {
    if (cachedPieceRadar)
        return cachedPieceRadar;
    if (pieceRadarPromise)
        return pieceRadarPromise;
    pieceRadarPromise = (async () => {
        try {
            const res = await fetch(assetUrl("assets/speak/piece-radar.json"));
            if (!res.ok)
                throw new Error(`piece-radar HTTP ${res.status}`);
            cachedPieceRadar = (await res.json());
            return cachedPieceRadar;
        }
        catch {
...
export function getPieceRadarAxes(pieceId, fallback) {
    if (!pieceId || !cachedPieceRadar?.axes?.[pieceId])
        return fallback;
    return cachedPieceRadar.axes[pieceId];
}
/** English labels for theme radar when UI locale is EN (assets store Chinese labels). */
export const THEME_LABEL_EN = {
    farewell: "Farewell",
    longing: "Longing",
    exam: "Exam / career",
...
export function localizeThemeAxes(axes, preferEn) {
    if (!preferEn)
        return axes;
    return axes.map((a) => ({ id: a.id, label: THEME_LABEL_EN[a.id] ?? a.label }));
}
function toPinyinSyllables(text, map) {
    const out = [];
...
function pinyinBigrams(syllables) {
    const out = new Set();
...
export function pickSceneCard(cards, pieceId, timeSec) {
    if (!cards || !pieceId)
        return null;
...
export function scoreHypAgainstIndex(hyp, index, topK = 5, pieceId, timeSec, windowSec = 50, pinyinMap) {
    ...
    for (const entry of index.entries) {
        if (pieceId && entry.pieceId !== pieceId)
            continue;
        ...
        if (useTime && entry.start != null && entry.end != null) {
            ...
            const dist = Math.abs(mid - timeSec);
            if (dist > windowSec) {
                if (score < 0.35)
                    continue;
                score *= 0.55;
...
function themeScoresFromMatches(matches, hyp) {
    const scores = Object.fromEntries(THEME_ORDER.map((t) => [t, 0]));
...
export function analyzeGist(hyp, index, opts = {}) {
    ...
    const matches = scoreHypAgainstIndex(hyp, index, 5, pieceId, timeSec, windowSec, opts.pinyinMap);
...
export { THEME_ORDER };
//# sourceMappingURL=speak-gist.js.map